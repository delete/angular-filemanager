(function() {
    'use strict';
    angular.module('FileManagerApp').service('fileNavigator', fileNavigator);
    
    fileNavigator.$inject = ['apiMiddleware', 'fileManagerConfig', 'item', 'navbarButtonsService'];
    /* @ngInject */
    function fileNavigator(ApiMiddleware, fileManagerConfig, Item, navbarButtonsService) {
        ...
        FileNavigator.prototype.refresh = function() {
            ...
            return self.list().then(function(data) {
                self.fileList = (data.result || []).map(function(file) {
                    // If the file has the .abkp extension, must change the type
                    var alterdataBkpRegex = /.*\.abkp$/;
                    if ( alterdataBkpRegex.test(file.name) ) {
                        file.type = 'abkp';                            
                    }
