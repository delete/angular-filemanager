(function() {
    'use strict';
    angular.module('FileManagerApp').service('apiMiddleware', apiMiddleware);

    apiMiddleware.$inject = ['$window', 'fileManagerConfig', 'apiHandler'];
        
    /* @ngInject */
    function apiMiddleware($window, fileManagerConfig, ApiHandler) {
        ...
        ApiMiddleware.prototype.restoreFromTrash = function(files) {
            var items = this.getFileList(files);
            return this.apiHandler.restoreFromTrash(fileManagerConfig.restoreUrl, items);
        };

        ApiMiddleware.prototype.downloadFromCloud = function(files) {
            var items = this.getFileList(files);
            return this.apiHandler.downloadFromCloud(fileManagerConfig.downloadFromCloudUrl, items);
        };
